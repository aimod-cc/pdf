...
```json
{
  ...
  "data": {
    "loggedIn": true,
    "user": { "id": 1, "username": "myaccount", "email": "user@example.com", "name": "昵称" }
  }
}
```
...
export function useAuth() {
  ...
  const register = () => {
    const returnUrl = encodeURIComponent(window.location.href);
    window.location.href = `${AUTH_BASE}/auth/register?redirect=${returnUrl}`;
  };
...
| 页面 | 地址 | 说明 |
|------|------|------|
| 登录/注册 | `/auth` | 公开页面，支持 `?redirect=` |
| 登录 | `/auth/login` | 公开页面，支持 `?redirect=` |
| 注册 | `/auth/register` | 公开页面，支持 `?redirect=` |
| 管理后台 | `/login` | 仅管理员 |
